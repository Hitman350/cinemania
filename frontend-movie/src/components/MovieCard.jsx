...
const MovieCard = ({ movie, onClick, layoutMode = 'grid' }) => {
  ...
  // Check if the poster URL is valid (doesn't end with "null")
  ...
  // Generate a random rating if not provided (for demo purposes)
  const rating = movie.rating || ((Math.random() * 3 + 7) ).toFixed(1);
        ...
        <div className="aspect-[2/3] overflow-hidden relative">
          ...
          {imageError ? (
            ...
            <img
              src={posterUrl}
              alt={movie.title}
              className={`movie-card-image w-full h-full object-cover transition-transform duration-700 ${imageLoaded ? 'opacity-100' : 'opacity-0'}`}
              ...
              {movie.genres && (
                <div className="flex flex-wrap gap-1">
                  {movie.genres.slice(0, 2).map((genre, index) => (
                    <span key={index} className="text-xs px-2 py-0.5 bg-gray-800/80 backdrop-blur-sm rounded-full text-cyan-300">
                      {genre}
                    </span>
                  ))}
                </div>
